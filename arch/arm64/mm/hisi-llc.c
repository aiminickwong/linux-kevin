...
/**
 * llc_writel - write LLC registers via djtag
 * @n:		CPU DIE ID
 * @offset:	LLC register's offset
 * @value:	value to write to LLC register
 */
static void llc_writel(struct device_node *node, u32 offset, u32 value)
{
	int ret;

	ret = djtag_writel(node, offset, 0x4, 0, value);
...
/**
 * llc_readl - read LLC registers via djtag
 * @n:		CPU DIE ID
 * @offset:	LLC register's offset
 * @bank:	bank number, mostly is 0
 *
 * return: register's value
 */
static u32 llc_readl(struct device_node *node, u32 offset, int bank)
{
	int ret;
	u32 val;

	ret = djtag_readl(node, offset, 0x4, bank, &val);
	if (ret) {
		pr_info("%s is not correct!\n", node->full_name);
		return 0;
	} else
...
static void llc_global_mnt(enum mnt_type type)
{
	...
	list_for_each_entry_safe(llc_data, p, &llc_list, list) {
		...
		llc_writel(llc_data->node, LLC_MAINT_CTRL, val);

		/* wait LLC maintain operation to be completed */
		while (llc_readl(llc_data->node, LLC_MAINT_CTRL, 0) & MNT_EN)
			cpu_relax();
	}
...
static void llc_range_mnt(enum mnt_type type, phys_addr_t start, size_t size)
{
	...
	for_each_mem_region(llc_data, p, r) {
		...
		get_addr_config(rend - rstart, size_l, size_h);

		/* write address and size to the correspond register */
		llc_writel(llc_data->node, LLC_MAINT_START_L, saddr_l);
		llc_writel(llc_data->node, LLC_MAINT_START_H, saddr_h);
		llc_writel(llc_data->node, LLC_MAINT_AREA_L, size_l);
		llc_writel(llc_data->node, LLC_MAINT_AREA_H, size_h);
 ...
 */
static void llc_setup_mem_map(struct llc_of_data *llc_data)
{
	u32 val;
	int i, die_id;
	phys_addr_t start;
	...
	for (i = 0; i < MAX_DAW; i++) {
		val = llc_readl(llc_data->node, LLC_DAW(i), 0);
		pr_debug("DAW %d\n", i);
...
static int llc_dev_probe(struct platform_device *pdev)
{
	...
	llc_data = kzalloc(sizeof(struct llc_of_data),
			GFP_KERNEL);
	if (llc_data == NULL) {
		pr_err("LLC: malloc llc_data error.\n");
		return -ENOMEM;
	}
	...
	val = llc_readl(llc_data->node, LLC_INVLD, 0);
