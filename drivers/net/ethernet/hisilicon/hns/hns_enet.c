			 ...
			 int buf_num, enum hns_desc_type type, int mtu)
{
	...
	hnae_set_field(bn_pid, HNSV2_TXD_BUFNUM_M,
		       HNSV2_TXD_BUFNUM_S, buf_num - 1);
	/* field port_id in the tx bd for sending manage pkgs.*/
...
static bool hns_nic_is_dup_pkt(struct sk_buff *skb, u8 *ndev_addr)
{
	struct ethhdr *eth_hdr = (struct ethhdr *)skb->data;

	/* check if pkt is unicast */
	if (likely(!(eth_hdr->h_dest[0] & 0x01)))
		return 0;

	/* check if smac_addr equals to local_mac_addr */
	if (memcmp(ndev_addr, eth_hdr->h_source, ETH_ALEN))
		return 0;
	else
		return 1;
}
...
recv:
	while (recv_pkts < budget && recv_bds < num) {
		...
		/* filter out duplicate multicast pkt */
		if (hns_nic_is_dup_pkt(skb, ndev->dev_addr)) {
			ring->stats.rx_pkts--;
			ring->stats.rx_bytes -= skb->len;
			recv_pkts++;
			dev_kfree_skb_any(skb);
...
static int hns_nic_set_features(
